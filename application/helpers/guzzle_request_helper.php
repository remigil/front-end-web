...
if (!function_exists('guzzle_request')) {
    ...
    { 
        try {
            ...
            $request = $client->request($method, 'http://k3ig20korlantas.id:3002/v1/' . $uri, $body); 
    ...
    function url_api()
    {
        ...
        return 'http://k3ig20korlantas.id:3002/uploads/'; 
    }
...
if (!function_exists('guzzle_requestTracking')) {
    ...
    function guzzle_requestTracking($method, $uri, $body)
    {
        try {
            ...
            $request = $client->request($method, 'http://k3ig20korlantas.id:3002/track-location/' . $uri, $body);
            $response = $request->getBody();
            $data = json_decode($response, true);
            return $data;
