<div class="container-fluid">
    <div class="row">
        ...
        <div class="col-md-6 text-end align-self-center">
            <a href="<?= ENV_API_BASE_URL ?>v1/laporan_harian/rekalantas" type="button" button class="btn btn-outline-primary btn-lg" style="width: 200px; border-color:#0007D8;">Export Laporan</a>
    ...
    <div class="card mt-5 p-1 shadow" style="border-radius:36px !important;">
        <div class="row m-2">
            <div class="col-sm-2 col-md-6 align-self-center">
                <h3>STATISTIK DATA <span style="text-transform:uppercase ; color:#0007D8">Rekalantas</span> Polda&nbsp;<?= $data['poldaid']['name_polda']; ?></h3>
            </div>
                            ...
                            <div class="card-body p-1">
                                <div class="row justify-content-between align-items-center" style="height: 80px;">
                                    ...
                                    <div class="col-md-5 float-end">
                                        ...
                                        <p class="text-center mb-0">Rekalantas</p>
                                    </div>
    ...
    <div class="modal-dialog modal-xl" role="document" style="min-width:70%;">
        <div class="modal-content">
            <div class=" modal-header">
                <h5 class="modal-title" id="modalLabelDay">Detail Rekalantas tanggal <?= format_indo(date('Y-m-d'))  ?> Seluruh Polda </h5>
                <!-- <h5 class="modal-title" id="modalLabelDay">Detail Rekalantas tanggal <?= format_indo(date('Y-m-d'))  ?> Seluruh Polda <span class="fs-5">sasdasdasd</span></h5> -->
                    ...
                    <table class="table table-bordered table-hover" id="tableLakaDay" style="background:white; ">
                        <thead style="background-color:#0007D8; color:#fff;">
                            <tr class="text-center">
                                <th scope="col" style="width:fit-content;">Ranking</th>
                                <th scope="col">Polda</th>
                                <th scope="col">Jalan Nasional</th>
                                <th scope="col">Jalan Provinsi</th>
...
<div class="modal fade " id="ModalMonth" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" style="background: rgba(0, 0, 0, 0.7);">
    <div class="modal-dialog modal-xl" role="document" style="min-width:70%;">
        <div class="modal-content">
            <div class=" modal-header">
                <h5 class="modal-title" id="modalLabelMonth">Detail Rekalantas Bulan <?= format_indo(date('Y-m'))  ?> Seluruh Polda</h5>
                <!-- <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                     <span aria-hidden="true">&times;</span>
                 </button> -->
            </div>
...
<div class="modal fade " id="ModalYear" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" style="background: rgba(0, 0, 0, 0.7);">
    <div class="modal-dialog modal-xl" role="document" style="min-width:70%;">
        <div class="modal-content">
            <div class=" modal-header">
                <h5 class="modal-title" id="modalLabelYear">Detail Rekalantas Tahun <?= format_indo(date('Y')) ?> Seluruh Polda</h5>
                <!-- <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                     <span aria-hidden="true">&times;</span>
                 </button> -->
            </div>
    ...
    <div class="card">
        ...
        <div class="row">
            <div class="col-xl-12">
                <div class="card">
                    <div class="card-header">
                        <div id="title">TOP DATA REKALANTAS LALU LINTAS</div>
                        <div class="row">
                            <div class="col-md-2">
                                <div class="form-group">
                                    ...
                                    <select class="form-control form-select" id="limit_showData">
                                        ...
                                        <option value="34">Keseluruhan Polda</option>
                                    </select>
                                </div>
                            </div>
                        </div>
...
<script>
    $(document).ready(function() {
        ...
        getBarRekalantas(filter, limit, firstDay, lastDay)
        getLineRekalantas(filter = 1, firstDay, lastDay, 'year')
        getStatistikRekalantas(filter = 1, yesterday, firstDay, lastDay, firstDayMonth, lastDayMonth)
        RekalantasToday(yesterday, yesterday)
        RekalantasMonth(firstDayMonth, lastDayMonth)
        RekalantasYear(firstDay, firstDay)
    ...
    $('#limit_showData').on('change', function() {
        ...
        if (start_date == '' && end_date == '') {
            getBarRekalantas(0, limit, 0, null, null)
        } else {
            getBarRekalantas(0, limit, 0, start_date, start_date)
        }
    ...
    function ButtonFilterOnTable(type) {
        if (type === 'day') {
            ...
            $('#modalLabelDay').text(`Detail Rekalantas Tanggal ${moment(start_date).format('DD MM YYYY')} s.d ${moment(end_date).format('DD MM YYYY')} Seluruh Polda`)
            RekalantasToday(start_date, end_date)
        } else if (type === 'month') {
            ...
            $('#modalLabelMonth').text(`Detail Rekalantas Bulan ${moment(start_date).format('MM YYYY')} s.d ${moment(end_date).format('MM YYYY')} Seluruh Polda`)
            RekalantasMonth(start_date, end_date)
        } else if (type === 'year') {
            ...
            $('#modalLabelYear').text(`Detail Rekalantas Tahun ${moment(start_date).format('YYYY')} s.d ${moment(end_date).format('YYYY')} Seluruh Polda`)
            RekalantasYear(start_date, end_date)
        }
    }


    function RekalantasToday(start_date, end_date) {
        $.ajax({
            type: "POST",
            url: "<?php echo base_url(); ?>executive/statistik_executive/rekalantasToday",
            dataType: "JSON",
            data: {
                start_date,
                end_date
            ...
            success: function(result) {
                ...
                for (let i = 0; i < result.length; i++) {
                    ...
                    table += `<tr class="text-center"> 
                        ...
                        <td>  ${result[i].jalan_nasional}  </td> 
                        <td>  ${result[i].jalan_provinsi}  </td> 
                        <td>  ${result[i].lain_lain}  </td> 
    ...
    function RekalantasMonth(start_date, end_date) {
        $.ajax({
            type: "POST",
            url: "<?php echo base_url(); ?>executive/statistik_executive/rekalantasMonth",
            dataType: "JSON",
            data: {
                start_date,
                end_date
    ...
    function RekalantasYear(start_date, end_date) {
        $.ajax({
            type: "POST",
            url: "<?php echo base_url(); ?>executive/statistik_executive/rekalantasYear",
            dataType: "JSON",
            data: {
                start_date,
                end_date
    ...
    function ButtonFilter() {
        ...
        if (start_date === '' || end_date === '') {
            ...
            getBarRekalantas(filter, limit, start_date, end_date)
            getLineRekalantas(filter, start_date, end_date, type)
        }
    ...
    function getStatistikRekalantas(filter, yesterday, firstDay, lastDay, firstDayMonth, lastDayMonth) {
        $.ajax({
            type: "POST",
            url: "<?php echo base_url(); ?>executive/statistik_executive/getRekalantasStatistik",
            dataType: "JSON",
            data: {
                yesterday,
                firstDayMonth,
    ...
    function getBarRekalantas(filter = null, limit = null, start_date = null, end_date = null) {
        $.ajax({
            type: "POST",
            url: "<?php echo base_url(); ?>executive/statistik_executive/getBarRekalantas",
            data: {
                filter: filter,
                limit: limit,
            ...
            dataType: "JSON",
            success: function(result) {
                $("#charta").html(`<div id="rekalantaschart"></div>`);
                console.log(result)

                var options_Rekalantas = {
                    series: [{
                        name: '<h6>Total Rekalantas</h6>',
                        type: 'column',
                        data: result.total,
                        color: "#11347A"
                    }, {
                        name: '<h6>Jalan Nasional</h6>',
                        type: 'column',
                        data: result.jalan_nasional,
                        color: "#CB2D3E"
                    }, {
                        name: '<h6>Jalan Provinsi</h6>',
                        type: 'column',
                        data: result.jalan_provinsi,
                        color: "#E8D42F"
                    }, {
                        name: '<h6>Lain Lain</h6>',
                        type: 'column',
                        data: result.lain_lain,
                        color: "#3CA55C"
                ...
                var Rekalantas = new ApexCharts(document.querySelector("#rekalantaschart"), options_Rekalantas);
                Rekalantas.render();
    ...
    function getLineRekalantas(filter = null, start_date = null, end_date = null, type = null) {
        $.ajax({
            type: "POST",
            url: "<?php echo base_url(); ?>executive/Statistik_executive/getLineRekalantas",
            data: {
                start_date: start_date,
                end_date: end_date,
                filter: filter,
            ...
            success: function(results) {
                ...
                var chart2 = {
                    series: [{
                        ...
                        name: '<h6>Jalan Nasional</h6>',
                        type: 'line',
                        data: results.jalan_nasional,
                        color: "#CB2D3E"
                    }, {
                        name: '<h6>Jalan Provinsi</h6>',
                        type: 'line',
                        data: results.jalan_provinsi,
                        color: "#E8D42F"
                    }, {
                        name: '<h6>Lain Lain</h6>',
                        type: 'line',
                        data: results.lain_lain,
                        color: "#3CA55C"
