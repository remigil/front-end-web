...
class M_tugasfungsi extends CI_Model
{
    ...
    public function get_datatables($postData = null)
    {
        ...
        $url = 'tugasfungsi?serverSide=True&length=' . $rowperpage . '&start=' . $page . '&order=' . $orderFieldRess . '&orderDirection=' . $orderValue . '' . $searchData . '';
        ...
        foreach ($result['data']['rows'] as $field) {
            ...
            $row['picture_satker'] = $field['picture_satker'];
            $row['youtube_satker'] = $field['youtube_satker'];
            $row['tugas_satker'] = $field['tugas_satker'];
            $row['fungsi_satker'] = $field['fungsi_satker'];

            $row['action'] = '   
            <a href="Tugasfungsi/edit/' . $field['id'] . '"><button style="background-color:transparent ; border:none" >
