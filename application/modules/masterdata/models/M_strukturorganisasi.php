...
class M_strukturorganisasi extends CI_Model
{
...

    {
        ...
        $url = 'strukturorganisasi?serverSide=True&length=' . $rowperpage . '&start=' . $page . '&order=' . $orderFieldRess . '&orderDirection=' . $orderValue . '' . $searchData . '';
        ...
        foreach ($result['data']['rows'] as $field) {
            ...
            // $row ['id']	=  $field['id']; 
            $row['id']    =  $no++;
            $row['tugas']      = $field['tugas'];
            $row['fungsi']           = $field['fungsi'];
            $row['action']             = '   
				...
				<button style="background-color:transparent ; border:none" data-bs-toggle="modal" id="btnEdit" onclick="detailEdit(`' . $field['id'] . '`)" data-bs-target=".Ubah">
