...
class M_publikasi extends CI_Model
{
    ...
    public function get_datatables($postData = null)
    {
        ...
        $url = 'publikasi?serverSide=True&length=' . $rowperpage . '&start=' . $page . '&order=' . $orderFieldRess . '&orderDirection=' . $orderValue . '' . $searchData . '';
        ...
        foreach ($result['data']['rows'] as $field) {
            ...
            // $row ['id']	=  $field['id']; 
            $row['id'] = $no++;
            $row['name_publikasi'] = $field['name_publikasi'];
            $row['description_publikasi'] = $field['description_publikasi'];
            $row['link_youtube_publikasi'] = $field['link_youtube_publikasi'];

            $row['action'] = '   
            <a href="Publikasi/edit/' . $field['id'] . '"><button style="background-color:transparent ; border:none" >
