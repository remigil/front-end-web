...
class M_negara extends CI_Model {
    ...
    {   
        ...
        $url = 'country?serverSide=True&length='.$rowperpage.'&start='.$page.'&order='.$orderFieldRess.'&orderDirection='.$orderValue.''.$searchData.'';
		...
		foreach  ($result['data']['data'] as $field) { 
            ...
            if($field['photo_country'] != null){
                $row ['photo_country'] = '<img src="'.url_api().'country/'.$field['photo_country'].'" alt="" class="rounded-circle avatar-md">';  
            }else{
            ...
            $row ['status_country']   			= '
                <div class="switch">
                    <input class="flag" type="checkbox" id="flag'.$field['id'].'" '.$stFlag.' data-id="'.$field['id'].'" data-name="'.$field['name_country'].'" data-status="'.$field['status_country'].'" data-toggle="toggle"  data-onstyle="success" data-offstyle="danger" data-on="Approved" data-off="Not Approved" data-size="lg"> 
                    <label for="flag'.$field['id'].'"></label>
                </div>
            ';


            // $row ['status_country']	= $field['status_country'];  

            $row ['action']         = ' 
                <a href="'.base_url().'masterdata/negara/edit/'.$field['id'].'"><button class="btn btn-sm btn-primary"><i class="mdi mdi-cog "></i></button></a>  
            '; 
